...
VBoxManage unattended install $VM_NAME --user=$USERNAME --password=$PASSWORD --country=US --time-zone=EST --language=en-US --hostname=$HOSTNAME --iso=$ISO_PATH --start-vm=gui --install-additions
...
echo "Creating a user account manually..."
VBoxManage guestcontrol $VM_NAME run --username $USERNAME --password $PASSWORD --exe "/usr/sbin/adduser" --wait-stdout --wait-stderr -- $USERNAME --gecos "" --disabled-password
VBoxManage guestcontrol $VM_NAME run --username $USERNAME --password $PASSWORD --exe "/usr/bin/passwd" --wait-stdout --wait-stderr -- $USERNAME --stdin <<< $PASSWORD